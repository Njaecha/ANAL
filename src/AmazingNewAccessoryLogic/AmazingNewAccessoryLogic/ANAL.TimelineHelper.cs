    ...
    internal class TimelineHelper {
        ...
        internal static void PopulateTimeline() {
            ...
            TimelineCompatibility.AddInterpolableModelDynamic(
                ...
                id: "groupState",
                name: "Group State",
                interpolateBefore: (oci, parameter, leftValue, rightValue, factor) => GetGroup(oci, parameter).state = leftValue,
                ...
                getFinalName: (currentName, oci, parameter) => $"{GetGroup(oci, parameter)?.getName() ?? "Unknown"} Group State",
                isCompatibleWithTarget: (oci) => oci is OCIChar ociChar && ociChar.charInfo == groupToAnimate?.ctrl.ChaControl
            );
        }
